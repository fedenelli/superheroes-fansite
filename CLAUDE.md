...
**Paper and label-maker titles** live in `src/styles/papel.css`, imported by Base. There are no white backgrounds: `body`, the footer and the pagination tiles' covers are a dotted drawing-paper sheet (`--papel-bg`). Section titles are `<Titulo>` (`src/components/Titulo.astro`), which prints the text on the global `.dymo` tape — the same class the tracklist uses for Lado A/B and the download button. Don't bring back `.sectionTitle` / `heading-t3` / `heading-b3`. Running prose (gracias, 404) goes in an `<article class="nota">`: an opaque photocopied sheet with a strip of tape, so the dots never sit behind body text.

**The release tracklist** lives in `src/components/Tracklist.astro` (the "fotocopia" look: a photocopied bootleg back cover, Lado A / Lado B, label-maker tapes). `player.ts` and the tests still find rows by `.trak-item`, `.play-pause-button` and `.trak-duration`, and map the nth row to the nth track, so the sides must keep dataset order. `.trak-item.active` gets the highlighter and `.playing` animates the bars. Its styles are `is:global`, namespaced `.shz-*`, and scoped under `.shz` so they out-rank and reset the legacy `.trak-item` rules still in `master.css`. The highlighter colour is each release's `accent` in `releases.ts`; black titles sit on it, so keep it light. Pages can add `<head>` tags through Base's `head` slot. Courier Prime is loaded site-wide by Base.
